...
export class LangSmithAdmin {
  ...
  async getPromptByName(promptName) {
    try {
      console.log(`Fetching prompt by name: ${promptName}`)
      
      // Try LangChain Hub first if it's a specific hub prompt
      if (promptName === 'chat-assistant') {
        try {
          console.log('Pulling chat assistant prompt from LangChain Hub...')
          const hubPrompt = await hub.pull("chat-assistant")
          
          const convertedPrompt = {
            id: 'chat-assistant',
            prompt_name: 'chat-assistant',
            prompt: hubPrompt,
            langchain_prompt: hubPrompt,
            tags: ['hub', 'chat-assistant', 'latest'],
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
            metadata: {
              version: 'latest',
              description: 'Chat assistant prompt from LangChain Hub',
              is_active: true,
              experiment_id: null,
              is_langchain_prompt: true,
            },
          }
          
          console.log('LangChain Hub chat assistant prompt loaded successfully')
          return convertedPrompt
          
        } catch (hubError) {
          console.log('LangChain Hub chat assistant not found, trying LangSmith API...', hubError.message)
        }
      }
      
      // Fallback to LangSmith API search
      const prompts = await this.client.listPrompts({
        query: promptName,
        limit: 50,
        is_archived: false,
      ...
      const matchingPrompt = promptsArray.find(p => 
        p.prompt_name === promptName || 
        p.prompt_name.toLowerCase() === promptName.toLowerCase()
      )
      
      if (matchingPrompt) {
        console.log(`Found prompt: ${matchingPrompt.prompt_name}`)
        return this.mapPrompt(matchingPrompt)
      }
      
      console.log(`No prompt found with name: ${promptName}`)
      return null
      
    } catch (error) {
      console.error(`Error fetching prompt by name ${promptName}:`, error)
      return null
    }
  }
    ...
    try {
      ...
      if (promptName === 'chat-assistant') {
        try {
          ...
          console.log('LangChain Hub chat assistant not found, trying LangSmith API...')
        }
      }
